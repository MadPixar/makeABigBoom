...
const stopRepeatedRequest = (url: string) => {
  if (reqList.includes(url)) return false
  reqList.push(url)
  return true
}
...
const type = (obj: any) =>
  Object.prototype.toString
    ...
    .slice(8, -1)
    .toLowerCase()
const request = axios.create({ timeout: 150000000 })

const request_noLoading = axios.create({ timeout: 15000 })

const pdf_preview_request = axios.create({ timeout: 90000 })
...
request.interceptors.request.use(
  config => {
    ...
    const flag = stopRepeatedRequest(url)
    if (!flag)
